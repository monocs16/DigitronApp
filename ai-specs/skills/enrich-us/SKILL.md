...
6. Save the enriched user story as a markdown file in the `tmp/` folder at the project root (create the folder if it doesn't exist). Name the file `<ticket-id>-enriched-us.md` (e.g., `LTI-42-enriched-us.md`). The `tmp/` folder is already in `.gitignore`.
