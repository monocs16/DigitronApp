...
import { PartFormDialog } from "@/components/parts-form-dialog";
import {
  transitionOrder,
  revertOrderStage,
...
function CollapsibleSectionCard({
  ...
  const [open, setOpen] = useState(openWhen ?? defaultOpen);

  useEffect(() => {
    if (openWhen !== undefined) setOpen(openWhen);
  }, [openWhen]);
    ...
    <Collapsible open={open} onOpenChange={setOpen} asChild>
      <Card className={className} data-testid={testId}>
        <CardHeader className="p-0">
          <CollapsibleTrigger className="group flex w-full items-center justify-between gap-3 rounded-t-xl p-6 text-left outline-none transition-colors hover:bg-muted/40 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        ...
        {onCreatePart && (
          <Button type="button" variant="outline" size="sm" onClick={onCreatePart}>
        ...
        <div className="flex items-end gap-2">
          <div className="min-w-0 flex-1">
            <Select value={partId} onValueChange={setPartId}>
              <SelectTrigger
                aria-label={stage === "quoted" ? t("orders.neededParts") : t("orders.usedParts")}
              >
...
function OrderDetailPage() {
  ...
  const { data: catalog = [] } = useQuery<CatalogPart[]>({
  ...
  const quotedPartIds = new Set(quotedParts.map((part) => part.part_id));
  const repairCatalog = catalog.filter((part) => quotedPartIds.has(part.id));
  ...
  const paidTotal =
    payments.reduce((s, p) => s + Number(p.amount), 0) + Number(budget?.advances ?? 0);
  ...
  const canCreateEvaluationPart =
    !isClosed && (isSuper || isAdministrativo || (isTecnico && isAssigned));
  ...
  const addPayment = useMutation({
    mutationFn: (data: PaymentValues) => {
      if (budgetForm.formState.isDirty) {
        throw new Error(i18n.t("orders.saveBudgetBeforePayment"));
      }
  ...
  const addPart = useMutation({
    mutationFn: (args: { stage: "quoted" | "used"; partId: string; qty: number }) => {
      ...
      if (args.stage === "used" && part.stock !== undefined && part.stock < args.qty) {
        throw new Error(i18n.t("orders.insufficientStock"));
      }
    ...
    onSuccess: (_res, args) => {
      ...
      qc.invalidateQueries({ queryKey: ["budget", orderId] });
      qc.invalidateQueries({ queryKey: ["audit", orderId] });
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const auditLabel = (a: (typeof audit)[number]) => {
    switch (a.table_name) {
      case "technical_evaluations":
        if (a.operation === "INSERT") return t("orders.evaluationAuditInsert");
        if (a.operation === "DELETE") return t("orders.evaluationAuditDelete");
        return t("orders.evaluationAuditUpdate");
      case "budgets":
        if (a.operation === "INSERT") return t("orders.budgetAuditInsert");
        if (a.operation === "DELETE") return t("orders.budgetAuditDelete");
        return t("orders.budgetAuditUpdate");
      case "repairs":
        if (a.operation === "INSERT") return t("orders.repairAuditInsert");
        if (a.operation === "DELETE") return t("orders.repairAuditDelete");
        return t("orders.repairAuditUpdate");
      case "payments":
        if (a.operation === "INSERT") return t("orders.paymentAuditInsert");
        if (a.operation === "DELETE") return t("orders.paymentAuditDelete");
        return t("orders.paymentAuditUpdate");
      case "order_parts":
        if (a.operation === "INSERT") return t("orders.partAuditInsert");
        if (a.operation === "DELETE") return t("orders.partAuditDelete");
        return t("orders.partAuditUpdate");
      case "parts":
        if (a.operation === "INSERT") return t("orders.inventoryPartAuditInsert");
        if (a.operation === "DELETE") return t("orders.inventoryPartAuditDelete");
        return t("orders.inventoryPartAuditUpdate");
      case "order_notes":
        if (a.operation === "INSERT") {
          const newRow = (a.full_row_new ?? {}) as Record<string, unknown>;
          const body = String(newRow.body ?? "");
          const deferredPrefix = "Motivo de diferimiento: ";
          if (body.startsWith(deferredPrefix)) {
            return t("orders.deferredReasonAudit", { reason: body.slice(deferredPrefix.length) });
          }
          return t("orders.noteAuditInsert");
        }
        if (a.operation === "DELETE") return t("orders.noteAuditDelete");
        return t("orders.noteAuditUpdate");
      case "order_photos":
        if (a.operation === "INSERT") return t("orders.photoAuditInsert");
        if (a.operation === "DELETE") return t("orders.photoAuditDelete");
        return t("orders.photoAuditUpdate");
    }

    if (a.operation === "INSERT") return t("orders.orderCreated");
    const changed = (a.changed_fields ?? {}) as Record<string, unknown>;
    if ("delivery_at" in changed || "received_by" in changed) {
      return t("orders.deliveryRecorded");
    }
    if ("stage" in changed) {
        ...
        return ownerAdmin ? (
          <div className="space-y-3">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="deferred-reason" className="text-xs">
                {t("orders.deferredReason")}
              </Label>
              <Input
                id="deferred-reason"
        ...
        return ownerAdmin ? (
          <div className="space-y-3">
            ...
            {!balanceSettled ? (
              ...
              <Button
                className="w-full"
                onClick={() => changeStage.mutate("awaiting_withdrawal")}
                disabled={changeStage.isPending}
              >
                {t("orders.markAwaitingWithdrawal")}
              </Button>
            )}
          </div>
        ) : (
      ...
      case "awaiting_withdrawal":
        return ownerAdmin ? (
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <Label htmlFor="received-by" className="text-xs">
                {t("orders.receivedBy")}
              </Label>
              <Input
                id="received-by"
                ...
                className="text-sm"
              />
              <Label htmlFor="closing-notes" className="text-xs">
                {t("orders.closingNotes")}
              </Label>
              <Textarea
                id="closing-notes"
                rows={2}
              ...
              <Button
                ...
                onClick={() => {
                  if (!receivedBy.trim()) {
                    toast.error(t("orders.receivedByRequired"));
                    return;
                  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="min-w-0 space-y-6 lg:col-span-2">
          ...
          <CollapsibleSectionCard
            ...
            title={t("orders.clientAndEquipment")}
            defaultOpen
          ...
          <CollapsibleSectionCard
            ...
            title={t("orders.evaluation")}
            trailing={<SectionPill status={evalStatus} t={t} />}
            openWhen={evalStatus === "active"}
            ...
            {evalStatus !== "future" && (
              <>
                ...
                <PartsEditor
                  ...
                  onCreatePart={
                    canCreateEvaluationPart && evalStatus === "active"
          ...
          {canSeeCommercial && (
            <CollapsibleSectionCard
              testId="budget-card"
              title={t("orders.budget")}
              trailing={<SectionPill status={budgetStatus} t={t} />}
              openWhen={budgetStatus === "active"}
                ...
                <Form {...budgetForm}>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between rounded-md border bg-muted/40 px-3 py-2 text-sm">
                      ...
                      <span className="font-semibold">{money(budgetDraftTotal)}</span>
                    </div>
          ...
          <CollapsibleSectionCard
            testId="repair-card"
            title={t("orders.repair")}
            trailing={<SectionPill status={repairStatus} t={t} />}
            openWhen={repairStatus === "active"}
            ...
            {repairStatus !== "future" && (
              <>
                ...
                <PartsEditor
                  ...
                  catalog={catalog}
                  selectableCatalog={repairCatalog}
                  onAddPart={(args) => addPart.mutate(args)}
                  onRemovePart={(id) => removePart.mutate(id)}
                  isAddingPart={addPart.isPending}
                  isRemovingPart={removePart.isPending}
          ...
          {canSeeCommercial && (
            <CollapsibleSectionCard
              ...
              trailing={<SectionPill status={paymentStatus} t={t} />}
              openWhen={paymentStatus === "active"}
              className={
            ...
            >
              {paymentStatus === "future" ? (
                ...
                <>
                  ...
                  {canEditPayments && order.stage === "payment" && (
                    <>
                      <Separator />
                      <Form {...paymentForm}>
                        <form
                        ...
                        >
                          ...
                          />
                          {budgetForm.formState.isDirty && (
                            <p className="text-xs text-amber-600 sm:col-span-4">
                              {t("orders.saveBudgetBeforePayment")}
                            </p>
                          )}
                          <Button
                            type="submit"
                            disabled={
                              addPayment.isPending || balance <= 0 || budgetForm.formState.isDirty
                            }
                          >
                            {t("orders.addPayment")}
                          </Button>
        ...
        <div className="space-y-6">
          <CollapsibleSectionCard
            testId="internal-notes-card"
            title={t("orders.internalNotes")}
          ...
          {canSeeCommercial && (
            <CollapsibleSectionCard testId="history-card" title={t("orders.history")}>
              {audit.length === 0 ? (
      ...
      <PartFormDialog
        ...
        hideCommercialFields={!canSeeCommercial}
        onSuccess={(id) => setEvalPartId(id)}
      />
      ...
      <AlertDialog open={confirmDelivery} onOpenChange={setConfirmDelivery}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("orders.confirmDelivery")}</AlertDialogTitle>
            <AlertDialogDescription>{t("orders.confirmDeliveryDesc")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t("common.cancel")}</AlertDialogCancel>
            <AlertDialogAction
